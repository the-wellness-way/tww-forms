...
class Test_SubConfirm_MeprTransaction extends \MeprTransaction {
    public function __construct() {
        parent::__construct([
            'id'              => 101,
            'amount'          => 11.99,
            ...
            'prorated'        => null,
            'created_at'      => date("Y-m-d H:i:s", time()),
            'expires_at'      => date("Y-m-d H:i:s", $this->tomorrow()), // should be yesterday
            'subscription_id' => 44,
            'order_id' => 1,
        ]);
    }
...
class TWW_Test_MembershipShortcode extends WP_UnitTestCase {
    ...
    /**
     * @covers subscription_latest_txn
     * @group membershipShortcode
     */
    public function test_subscription_latest_txn() {
     ...
     * @group membershipShortcode
     */
    public function test_subscription_latest_txn_without_txn() {
        $twwMemberShipMock = $this->getMockBuilder(TWW_MembershipShortcode::class)
            ->setConstructorArgs([null, $this->subscription_without_txn, null])
            ->onlyMethods(['subscription_latest_txn'])
            ->getMock();

        $twwMemberShipMock->expects($this->once())
            ->method('subscription_latest_txn')
            ->willReturn(null);

        $this->assertEquals($twwMemberShipMock->subscription_latest_txn(), null);
    }
    ...
    public function test_expired_scenario_produces_expired_string() {
        $mock = $this->getMockBuilder(Tww_MembershipShortcode::class)
        ->setConstructorArgs([null, $this->subscription, $this->expired_transaction])
        ->onlyMethods(['subscription_latest_txn', 'subscription_is_expired', 'subscription_in_grace_period'])
        ->getMock();

        $mock->expects($this->once())
        ->method('subscription_latest_txn')
        ->willReturn(new \Test_Expired_MeprTransaction());

        $mock->expects($this->once())
        ->method('subscription_is_expired')
        ->willReturn(true);

        $mock->expects($this->once())
        ->method('subscription_in_grace_period')
        ->willReturn(false);

        $this->assertEquals('<span class="status-tag expired">Expired</span>', $mock->print_status_tag());
    }
}
